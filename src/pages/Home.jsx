      ...
      <main className="flex-grow py-12 px-4 sm:px-6 md:px-12">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="bg-[#1B2A49] rounded-2xl p-6 shadow-xl hover:shadow-2xl transition">
            ...
            <h2 className="text-3xl font-bold mb-5 text-white text-center">Tickets</h2>
        ...
        <div className="mt-12 max-w-7xl mx-auto flex justify-center gap-4">
          <button
            ...
            className={`px-6 py-2 rounded-full font-bold ${activeFilter === "hotels" ? "bg-[#8B5E3C]" : "bg-[#1B2A49]"} hover:bg-[#8B5E3C] transition`}
          >
            Hotels
          </button>
          <button
            onClick={() => setActiveFilter("hostels")}
            className={`px-6 py-2 rounded-full font-bold ${activeFilter === "hostels" ? "bg-[#8B5E3C]" : "bg-[#1B2A49]"} hover:bg-[#8B5E3C] transition`}
          >
            Hostels
          </button>
        </div>
        ...
        <section className="mt-8 max-w-7xl mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {(activeFilter === "hotels" ? hotels : hostels).map((place, i) => (
              <div
                key={i}
                className="bg-[#1B2A49] rounded-xl shadow-md p-5 border border-yellow-400 hover:shadow-lg hover:bg-[#8B5E3C] hover:text-white transition-all"
              >
                <h3 className="text-xl font-semibold mb-1">{place.name}</h3>
                <p className="text-gray-300 mb-2">{place.address}</p>
                <p className="text-gray-400 text-sm mb-3">{place.type} • {place.distance}</p>
                <div className="flex justify-between">
                  <a href={place.url} target="_blank" rel="noopener noreferrer" className="text-blue-400 font-semibold hover:underline">Website</a>
                  <a href={place.directions} target="_blank" rel="noopener noreferrer" className="text-blue-400 font-semibold hover:underline">Directions</a>
                </div>
              </div>
            ))}
          </div>
